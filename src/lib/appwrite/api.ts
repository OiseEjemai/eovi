...
export async function getUserById(userId: string) {
  try {
    ...
    if (!user || !user.$id || !user.username) {
      throw new Error(`Invalid user data received for ID: ${userId}`);
    }
...
export async function unfollowUser(currentUserId: string, targetUserId: string) {
  try {
    ...
    throw new Error("Unable to unfollow user. Please try again.");
  }
}
...
export const createComment = async (
  ...
  const currentTimestamp = new Date().toISOString(); // Generate ISO 8601 timestamp
  return databases.createDocument(
    appwriteConfig.databaseId, // Replace with your database ID
    appwriteConfig.commentCollectionId, // Replace with your comments collection ID
    ID.unique(), // Auto-generate a unique ID
    { post_id: postId, user_id: userId, content, timestamp: currentTimestamp }
  );
};
...
export const getCommentsByPost = async (postId: string) => {
  return databases.listDocuments(
    ...
    appwriteConfig.commentCollectionId, // Replace with your comments collection ID
    [
      Query.equal("post_id", postId), // Filter by the related post ID
    ]
  );
};